...
import streamlit.components.v1 as components  # 👈 修正：確保 components 已定義
import pandas as pd
import sqlite3
import random
import re
...
def auto_sync_logic():
    ...
    if need_sync:
        try:
            ...
            conn.commit()
            st.sidebar.caption(f"🔄 自動同步完成 ({today})")
        except:
            pass
    conn.close()
...
# ... (get_weighted_question 與 get_cloze_question 保持原封不動) ...
...
def create_audio_button(text, button_text, theme_mode):
    ...
    try:
        ...
        style="width:100%;padding:10px;border-radius:10px;background:{bg_color};color:{text_color};border:none;">
        {button_text}</button>
        """
        components.html(html_code, height=50)
    except: st.warning("發音失敗")
...
# ==============================================================================
# INIT SESSION & AUTO SYNC
# ==============================================================================
init_session()
auto_sync_logic() # 👈 啟動自動同步檢查

# ==============================================================================
# 需求 4：UI 美化 (CSS)
# ==============================================================================
card_bg = "#111827" if theme_mode == "深色" else "#ffffff"
text_color = "white" if theme_mode == "深色" else "#1f2937"
border_style = "none" if theme_mode == "深色" else "1px solid #e5e7eb"
...
.big {{ font-size: 24px; color: {text_color}; font-weight: 600; line-height: 1.5; }}
.banner-img {{ width: 100%; height: 150px; object-fit: cover; border-radius: 15px; margin-bottom: 15px; }}
</style>
...
if mode == "測驗":
    ...
    # 👈 需求 2：填空模式時，display_text 優先抓取 sentence 且不含中文
    if practice_mode == "填空":
    ...
    if st.session_state.state == "q":
        for opt in q["options"]:
            if st.button(opt, use_container_width=True): # 使用全寬按鈕更美觀
        ...
        # 👈 需求 1：學習紀錄會留存在資料庫中 (你原本的邏輯已包含 SQLite 寫入則會更好)
        if not is_correct:
            st.session_state.wrong_list.append(q)
            # 在這裡其實可以增加一行 SQLite 寫入來增加 wrong_count
    ...
    with st.form("add"):
        ...
        if st.form_submit_button("送出"):
            payload = {"method": "write", "word": w, "definition": d, "example": ex, "point": pt}
